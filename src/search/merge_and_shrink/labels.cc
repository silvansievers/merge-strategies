...
bool Labels::is_active_label(int label_no) const {
    assert(utils::in_bounds(label_no, label_costs));
    return label_costs[label_no] != -1;
}
...
bool Labels::operator==(const Labels &other) const {
    return label_costs == other.label_costs &&
        max_num_labels == other.max_num_labels &&
        num_active_labels == other.num_active_labels;
}
}
