...
vector<vector<set<FactPair>>> g_mutexes;
...
vector<vector<set<FactPair>>> read_mutexes(istream &in, const vector<ExplicitVariable> &variables) {
    ...
    g_mutex_var_pairs.resize(variables.size(),
                             vector<bool>(variables.size(), false));
        ...
        for (const FactPair &fact1 : invariant_group) {
            for (const FactPair &fact2 : invariant_group) {
                if (fact1.var != fact2.var) {
                    ...
                    g_mutex_var_pairs[fact1.var][fact2.var] = true;
                    g_mutex_var_pairs[fact2.var][fact1.var] = true;
                }
            }
        }
    }
...
RootTask::RootTask(istream &in) {
    ...
    g_mutexes = read_mutexes(in, variables);
...
bool RootTask::are_facts_mutex(const FactPair &fact1, const FactPair &fact2) const {
    ...
    assert(utils::in_bounds(fact1.var, g_mutexes));
    assert(utils::in_bounds(fact1.value, g_mutexes[fact1.var]));
    return bool(g_mutexes[fact1.var][fact1.value].count(fact2));
}
